...
Два режими на урок:
- **Урок** — послідовний, гід «з нуля»; кнопка «↺ Почати спочатку» скидає
  прогрес уроку, щоб пройти його знову.
- **Практика** — нескінченний банк незалежних завдань (кожне запускається
  у свіжому шеллі, тому їх можна повторювати в будь-якому порядку).
  Рівень складності підбирається адаптивно (важче після серії успіхів,
  легше після серії помилок), а неправильно виконані завдання
  повертаються на повторення раніше за інші (spaced repetition).

Після кожної спроби, що не відповідає задачі, застосунок показує
ненав'язливу підказку категорії помилки (шлях не знайдено, немає прав
тощо), не розкриваючи саму відповідь — для цього є окремі кнопки
«Підказка» і «Відповідь» (зі знижкою XP за їх використання).
